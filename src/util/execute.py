...
def execute(model_name, decoding_strategy, dataset_name, dataset_split, batch_size, sink_tokens,
    ...
    results_path = f'{model_name}_{dataset_name}_{decoding_strategy.name.lower()}_{str(sink_tokens)}_{str(retention_window_length)}_{str(skip_prefill_compression)}_{seq_pooling_type.name.lower()}'
